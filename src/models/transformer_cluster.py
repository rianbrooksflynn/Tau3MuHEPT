...
def prepare_input(x, coords, batch, helper_params):
    ...
    with torch.no_grad():
        ...
        for graph_idx in range(len(graph_size_cumsum)):
            ...
            #print(sorted_eta_idx)
            #print(sorted_phi_idx)
            #print()
            #print(regions[0][:,None])
...
class TransformerCluster(nn.Module):
    def __init__(self, in_dim, coords_dim, **kwargs):
        ...
        self.comparator = nn.parameter.Parameter(torch.normal(torch.zeros(self.out_dim),1),requires_grad=True)
    ...
    def forward(self, x, coords, batch, pool=True):
        if not self.baseline:
            ...
            if pool:
                out = self.out_proj(global_mean_pool(encoded_x[unpad_seq], batch))
            else:
                out = self.out_proj(encoded_x[unpad_seq])
