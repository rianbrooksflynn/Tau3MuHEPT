...
class Tau3MuGNNs:
    ...
    def __init__(self, config, device, log_path, setting):
        ...
        self.criterion = ClusterLoss(config['optimizer'])
    ...
    def eval_one_batch(self, pos_batch, new_epoch=None):
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                acc = torch.sum(labels==targets)/len(targets)
                loss_dict['total_accuracy'] = acc
                
                sig_acc = torch.sum(labels[targets==1]==targets[targets==1])/torch.sum(targets==1)
                bkg_acc = torch.sum(labels[targets==0]==targets[targets==0])/torch.sum(targets==0)
                
                loss_dict['signal_accuracy'] = sig_acc
                loss_dict['bkg_accuracy'] = bkg_acc
